...
if __name__ == "__main__":
    ...
    df_coupon_unsaved = pd.read_sql("SELECT ul.user_id, ul.action_id AS coupon_id, c.text AS coupon_text, "
                                    "ul.time, "
                                    ...
                                    "WHERE ul.coupon_unsaved = True and CAST(ul.time as DATE) <= now() ", con=con_prod)
    df_coupon_unsaved = df_coupon_unsaved[df_coupon_unsaved['user_id'].isin(installed_users)]
    df_coupon_unsaved = df_coupon_unsaved[['user_id', 'coupon_id', 'time', 'category_id', 'retailer_id',
                                           'retailer_status', 'standard_coupon', 'discount', 'discount_y']]
    df_coupon_unsaved['discount_y'].fillna(0, inplace=True)
    print('The total number of users who unsaved coupons', len(df_coupon_unsaved['user_id'].unique().tolist()))
    print('The total number of unsaved coupons are', len(df_coupon_unsaved['coupon_id'].unique().tolist()))

    print('Concatenating data for bulk insert')
    data = [tuple(row) for index, row in df_coupon_unsaved.iterrows()]
    data = tuple(data)

    cursor_prod.close()

    print('Dropping data from the table before bulk inserting since it is faster than bulk update or bulk upsert')
    delete_coupon_unsaved(config_prod)

    print('Bulk inserting unsaving coupons data')
    insert_coupon_unsaved(data, config_prod)
