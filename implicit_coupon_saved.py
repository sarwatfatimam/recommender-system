...
if __name__ == "__main__":
    ...
    # Extract users information after removing invalid accounts
    ...
    df_coupon_saved = pd.read_sql("SELECT ul.user_id, ul.action_id AS coupon_id, c.text AS coupon_text, "
                                  "ul.time, "
                                  ...
                                  "WHERE ul.coupon_saved = True and CAST(ul.time as DATE) <= now() ", con=con_prod)

    df_coupon_saved = df_coupon_saved[df_coupon_saved['user_id'].isin(installed_users)]
    df_coupon_saved = df_coupon_saved[['user_id', 'coupon_id', 'time', 'category_id', 'retailer_id',
                                       'retailer_status', 'standard_coupon', 'discount', 'discount_y']]
    df_coupon_saved['discount_y'].fillna(0, inplace=True)
    print('The total number of users who saved coupons', len(df_coupon_saved['user_id'].unique().tolist()))
    print('The total number of saved coupons are', len(df_coupon_saved['coupon_id'].unique().tolist()))

    print('Concatenating data for bulk insert')
    data = [tuple(row) for index, row in df_coupon_saved.iterrows()]
    data = tuple(data)

    cursor_prod.close()

    print('Dropping data from the table before bulk inserting since it is faster than bulk update or bulk upsert')
    delete_coupon_saved(config_prod)

    print('Bulk inserting saving coupons data')
    insert_coupon_saved(data, config_prod)
